extends Node2D
## Manages the Old Man's cave where the player receives the wooden sword.
...
# Cave tile layout (from screen 0,8 of overworld.json)
...
@onready var old_man: Node2D = $OldMan
...
func exit_cave() -> void:
	## Return to the overworld at the starting screen
	player_entered = false  # Prevent multiple triggers

	# Set return state in GameManager
	GameManager.returning_from_cave = true
	GameManager.cave_exit_screen = Vector2i(7, 7)
	GameManager.cave_exit_tile = Vector2i(4, 1)  # Col 5, Row 2 in 1-based
